...
/**
 * Derived data function which returns the first number raised to the power of
 * the second number.
 * 
 * @author AWE Plc copyright 2013
 */
public final class Pow extends AbstractFunction {
	...
	protected String apply(DataGrid dataGrid, List<String> argCols) {
		...
		final String name = "pow(" + argCols.get(0) + ", " + argCols.get(1)
				+ ")";
		for (DataGrid.Row row : dataGrid.getRows()) {
			final Value<Double> val = Value.doubleValue(row.getyValue(argCols
					.get(0)));
			final Value<Double> exp = Value.doubleValue(row.getyValue(argCols
					.get(1)));
			row.addyValue(name, val.updateValue(Math.pow(val.getValue(), exp
					.getValue())));
		}
		return name;
	}
